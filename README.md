...
<!-- Please read https://github.com/linuxserver/docker-nzbhydra2/blob/dev/.github/CONTRIBUTING.md -->
...
Simply pulling `lscr.io/linuxserver/nzbhydra2:dev` should retrieve the correct image for your arch, but you can also pull specific arch images via tags.
...
docker run -d \
  ...
  --restart unless-stopped \
  lscr.io/linuxserver/nzbhydra2:dev
```
...
* Image version number:
    ...
    ```bash
    docker inspect -f '{{ index .Config.Labels "build_version" }}' lscr.io/linuxserver/nzbhydra2:dev
    ```
...
* Update the image:
    ...
    ```bash
    docker pull lscr.io/linuxserver/nzbhydra2:dev
    ```
...
docker build \
  --no-cache \
  --pull \
  -t lscr.io/linuxserver/nzbhydra2:dev .
```
...
* **18.07.24:** - Rebase to Ubuntu Noble. Remove standalone JRE ([#47](https://github.com/linuxserver/docker-nzbhydra2/pull/47)).
